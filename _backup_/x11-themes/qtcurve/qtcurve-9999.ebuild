...
src_prepare() {
	cp -r gtk2 gtk3
	rm -rf gtk2
	find . -type f -print0 | xargs -0 sed -i 's/gtk-2/gtk-3/g' || die
	find . -type f -print0 | xargs -0 sed -i 's/gtk+-2.0/gtk+-3.0/g' || die
	find . -type f -print0 | xargs -0 sed -i 's/gtk2/gtk3/g' || die
	find . -type f -print0 | xargs -0 sed -i 's/GTK2/GTK3/g' || die
}
...
src_configure() {
	...
	mycmakeargs=(
		...
		$(cmake-utils_use_enable gtk GTK3)
