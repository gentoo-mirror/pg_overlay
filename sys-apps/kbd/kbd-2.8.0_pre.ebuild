...
inherit multiprocessing poly-c_ebuilds
...
if [[ ${MY_PV} == 9999 ]] ; then
	...
	if [[ $(ver_cut 3) -lt 90 ]] ; then
		SRC_URI="https://www.kernel.org/pub/linux/utils/kbd/${MY_P}.tar.xz"
		KEYWORDS="~alpha ~amd64 ~arm ~arm64 ~hppa ~loong ~m68k ~mips ~ppc ~ppc64 ~riscv ~s390 ~sparc ~x86"
	else
		inherit autotools
		SRC_URI="https://github.com/legionus/kbd/archive/v${MY_PV}.tar.gz -> ${MY_P}.tar.gz"
	fi
fi
...
src_prepare() {
	...
	if [[ ${MY_PV} == 9999 ]] || [[ $(ver_cut 3) -ge 90 ]] ; then
		eautoreconf
	fi
}
