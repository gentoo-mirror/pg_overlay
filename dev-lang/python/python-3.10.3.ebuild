...
src_compile() {
	...
	if use pgo ; then
		...
		emake profile-opt PROFILE_TASK="-m test -x test_doctest test_bdb test_support test_compileall test_ftplib test_import test_importlib test_runpy test_pickle test_ssl test_socket -j $(nproc) --pgo-extended"
	else
