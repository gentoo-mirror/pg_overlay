...
src_prepare() {
	if use server ; then
		...
		rm "${FILESDIR}"/*_server_*
	fi

	EPATCH_SOURCE="${FILESDIR}" EPATCH_SUFFIX="patch" EPATCH_EXCLUDE="*100*" \
		EPATCH_FORCE="yes" epatch

	if use server ; then
		cd unix/xserver
		epatch "${FILESDIR}"/1000_server_xserver-1.16-rebased.patch
		epatch "${FILESDIR}"/1005_server_xserver-1.17.patch
		eautoreconf
	fi
}
...
src_configure() {
	...
	if use server; then
		...
		econf \
			...
			--without-dtrace \
			--disable-present \
			--disable-unit-tests --disable-ipv6
	fi
}
