...
inherit user-info flag-o-matic autotools pam systemd toolchain-funcs verify-sig poly-c_ebuilds
...
SCTP_VER="1.2" SCTP_PATCH="${PN}-8.9p1-sctp-${SCTP_VER}.patch.xz"
X509_VER="13.3.1" X509_PATCH="${PN}-8.9p1+x509-${X509_VER}.diff.gz"
...
src_prepare() {
	...
	if use X509 ; then
		...
		eapply "${FILESDIR}/${PN}-8.9_p1-X509-glue-"${X509_VER}".patch"
		popd &>/dev/null || die
	...
	if use hpn ; then
		local hpn_patchdir="${T}/${MY_P}-hpn${HPN_VER}"
