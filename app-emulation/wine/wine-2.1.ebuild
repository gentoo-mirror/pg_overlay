...
if [[ ${PV} == "9999" ]] ; then
	...
	MAJOR_V=$(get_version_component_range 1-1)
	MY_P="${P/_/-}"
...
if [[ ${PV} == "9999" ]] ; then
	...
	SRC_URI="${SRC_URI}
	staging? ( https://github.com/${PN}-compholio/${PN}-patched/archive/staging-${PV}.tar.gz -> ${STAGING_P}.tar.gz )"
fi
...
wine_env_vcs_vars() {
	...
	if [[ ! -z ${EGIT_COMMIT} ]]; then
		...
		eerror "WINE_COMMIT and STAGING_COMMIT"
		eerror
		return 1
	fi
}
