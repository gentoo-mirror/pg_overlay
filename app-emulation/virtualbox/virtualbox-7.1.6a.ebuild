...
# $Id: 9fc36665c23af44a47f240c40cb6d5255f49d804 $
...
COMMON_DEPEND="
	...
	|| (
		...
		~app-emulation/virtualbox-modules-${BASE_PV}_pre
	)
	dev-libs/libtpms
	dev-libs/libxml2
	dev-libs/openssl:0=
...
src_prepare() {
	...
	if use python ; then
		mkdir test || die
		cp "${FILESDIR}"/test_python.py test/ || die
...
src_install() {
	...
	if use python ; then
		...
		sed -e '1 i #! /usr/bin/python3' -i vboxshell.py || die
