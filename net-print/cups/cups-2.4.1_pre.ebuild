...
inherit autotools flag-o-matic linux-info xdg multilib-minimal optfeature pam systemd toolchain-funcs poly-c_ebuilds

REAL_PV="${MY_PV/_beta/b}"
REAL_PV="${REAL_PV/_rc/rc}"
REAL_PV="${REAL_PV/_p/op}"
REAL_P="${PN}-${REAL_PV}"
...
#	EGIT_REPO_URI="https://github.com/apple/cups.git"
	...
	[[ ${MY_PV} != 9999 ]] && EGIT_BRANCH=branch-${MY_PV/.9999}
else
#	SRC_URI="https://github.com/apple/cups/releases/download/v${REAL_PV}/${REAL_P}-source.tar.gz"
	SRC_URI="https://github.com/OpenPrinting/cups/releases/download/v${REAL_PV}/cups-${REAL_PV}-source.tar.gz"
	if [[ "${MY_PV}" != *_beta* ]] && [[ "${MY_PV}" != *_rc* ]] ; then
