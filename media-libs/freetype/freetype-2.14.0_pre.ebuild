...
if [[ ${MY_PV} == 9999 ]] ; then
	...
	SRC_URI="
		...
		utils? (
			https://downloads.sourceforge.net/freetype/ft2demos-${MY_PV}.tar.xz
			mirror://nongnu/freetype/ft2demos-${MY_PV}.tar.xz
		)
		doc? (
			https://downloads.sourceforge.net/freetype/${PN}-doc-${MY_PV}.tar.xz
			mirror://nongnu/freetype/${PN}-doc-${MY_PV}.tar.xz
		)
	"
...
_egit_repo_handler() {
	if [[ ${MY_PV} == 9999 ]] ; then
		...
		if use utils ; then
			...
			local EGIT_CHECKOUT_DIR="${WORKDIR}/ft2demos-${MY_PV}"
			git-r3_src_${phase}
		fi
	else
		default
...
multilib_src_install() {
	...
	if multilib_is_native_abi && use utils ; then
		...
		emake DESTDIR="${D}" FT2DEMOS=1 \
			TOP_DIR_2="${WORKDIR}/ft2demos-${MY_PV}" install
	fi
}
