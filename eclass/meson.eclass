...
inherit multiprocessing samu-utils python-utils-r1 toolchain-funcs
...
meson_src_test() {
	...
	[[ -n ${SAMUOPTS} || -n ${MAKEOPTS} ]] &&
		mesontestargs+=(
			--num-processes "$(makeopts_jobs ${SAMUOPTS:-${MAKEOPTS}})"
		)
...
# @USAGE: [extra samu install arguments]
# @DESCRIPTION:
# This is the meson_src_install function.
meson_src_install() {
	debug-print-function ${FUNCNAME} "$@"

	DESTDIR="${D}" esamu -C "${BUILD_DIR}" install "$@"
	einstalldocs
}
