...
# At this point only "emake" and "samu" are supported.
# In EAPI 7 and above, the default is set to "samu",
...
inherit toolchain-funcs samu-utils flag-o-matic multiprocessing xdg-utils
...
_cmake_generator_to_use() {
	...
	case ${CMAKE_MAKEFILE_GENERATOR} in
		samu)
			# if samu is enabled but not installed, the build could fail
...
# @FUNCTION: _cmake_samu_src_make
# @INTERNAL
# @DESCRIPTION:
# Build the package using samu generator
