...
COMMON_DEPEND="
	!libav? ( >=media-video/ffmpeg-4.0.0:0=[encode?,threads,vaapi?,vdpau?] )
	libav? ( >=media-video/libav-13:0=[encode?,threads,vaapi?,vdpau?] )
...
src_prepare() {
	...
	chmod +x "${S}"/waf || die
	sed -i 's/1.9.8/1.9.15/g' bootstrap.py || die
