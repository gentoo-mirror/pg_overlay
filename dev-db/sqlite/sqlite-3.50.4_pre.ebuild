...
inherit flag-o-matic multilib multilib-minimal toolchain-funcs poly-c_ebuilds
...
if [[ ${MY_PV} == 9999 ]]; then
	...
	printf -v SRC_PV "%u%02u%02u%02u" $(ver_rs 1- " " ${MY_PV})
	DOC_PV="${SRC_PV}"
	#printf -v DOC_PV "%u%02u%02u00" $(ver_rs 1-3 " " ${MY_PV})
...
multilib_src_compile() {
	emake HAVE_TCL="$(usex tcl 1 0)" TCLLIBDIR="${EPREFIX}/usr/$(get_libdir)/${MY_P}"
...
multilib_src_install() {
	...
	if [[ ${CHOST} == *-darwin* ]] ; then
		...
		install_name_tool \
			...
			"${ED}/usr/$(get_libdir)/libsqlite3$(get_libname ${MY_PV})" \
