...
WATERFOX_DIR="%WATERFOX_DIR%"
WATERFOX_FILE="%WATERFOX_NAME%"

if [[ ! -r ${MOZ_LIB_DIR}/${WATERFOX_DIR}/${WATERFOX_FILE} ]]; then
	if [[ ! -r ${SECONDARY_LIB_DIR}/${WATERFOX_DIR}/${WATERFOX_FILE} ]]; then
		echo "Error: ${MOZ_LIB_DIR}/${WATERFOX_DIR}/${WATERFOX_FILE} not found" >&2
		if [[ -d ${SECONDARY_LIB_DIR} ]]; then
			echo "       ${SECONDARY_LIB_DIR}/${WATERFOX_DIR}/${WATERFOX_FILE} not found" >&2
		fi
		exit 1
	fi
	MOZ_LIB_DIR="${SECONDARY_LIB_DIR}"
fi
MOZILLA_FIVE_HOME="${MOZ_LIB_DIR}/${WATERFOX_DIR}"
MOZ_EXTENSIONS_PROFILE_DIR="${HOME}/.mozilla/extensions/{ec8030f7-c20a-464f-9b0e-13a3a9e97384}"
MOZ_PROGRAM="${MOZILLA_FIVE_HOME}/${WATERFOX_FILE}"
