...
src_prepare() {
	...
	sed -i -e 's|use_lld=false|use_lld=true|' src/buildtools/config/linux.pri
	sed -i -e 's|is_clang=false|is_clang=true|' src/buildtools/config/linux.pri
	sed -i -e 's|clang_use_chrome_plugins=true|clang_use_chrome_plugins=false|' src/buildtools/config/linux.pri
